...
const BookSuggestions = () => {
  ...
  // categories no longer needed here; form moved to dedicated page
  ...
  useEffect(() => {
    if (success && message) {
      // Refresh suggestions list and clear banner after a moment
      dispatch(getAllSuggestions({ status: statusFilter, sortBy }))
      setTimeout(() => {
  ...
  const hasUserVoted = (suggestion) => {
    if (!suggestion?.votes || !user?._id) return false
    return suggestion.votes.some(v => String(v) === String(user._id))
  }

  // Remove any legacy "Condition: ..." lines from older suggestions' descriptions
  const sanitizeDescription = (desc) => {
    if (!desc) return ''
    return desc
      .split('\n')
      .filter((line) => !/^\s*condition\s*:/i.test(line))
      .join('\n')
  }

  // Form removed; dedicated SuggestBook page handles creation
  ...
  return (
    <div className="space-y-6">
                ...
                <div className="flex-1">
                  <div className="flex items-start">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-gray-700 mb-3">{sanitizeDescription(suggestion.description)}</p>
      ...
      {/* Floating Suggest Button (always visible) */}
      <button
        type="button"
        onClick={() => navigate('/suggest-book')}
        className="fixed bottom-6 right-6 bg-blue-600 text-white rounded-full shadow-lg p-4 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
        aria-label="Suggest a Book"
        title="Suggest a Book"
      >
        <Plus className="h-6 w-6" />
      </button>
      {/* Form modal removed; dedicated page handles suggestion creation */}
    </div>
  )
}
